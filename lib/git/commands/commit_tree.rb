...
module Git
  module Commands
    ...
    class CommitTree < Git::Commands::Base
      ...
      #       have no parents.
      #
      #     @option options [Boolean, String] :gpg_sign (nil) sign the
      #       commit with GPG
      #
      #       When `true`, signs with the default key. When a string, uses that
      #       key ID. When `false`, adds `--no-gpg-sign` to countermand an
      #       earlier `--gpg-sign`.
      #
      #       Alias: :S
      #
