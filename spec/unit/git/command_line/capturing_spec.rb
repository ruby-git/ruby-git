...
RSpec.describe Git::CommandLine::Capturing do
  ...
  # Builds a mock ProcessExecuter::ResultWithCapture.
  ...
  def mock_result(**overrides)
    ...
    double('ProcessExecuter::ResultWithCapture', defaults.merge(overrides))
  end

  describe '#run' do
    subject(:result) { described_instance.run(*run_args, **run_opts) }

    let(:run_args) { ['version'] }
    let(:run_opts) { {} }

    context 'with no options' do
      before do
        mocked = mock_result(command: %w[git version], stdout: "git version 2.39.1\n")
    ...
    context 'with chomp: true' do
      ...
      it 'chomps stdout and stderr' do
        expect(result.stdout).to eq('git version 2.39.1')
    ...
    context 'with normalize: true' do
      ...
      context 'when output is already valid UTF-8' do
        before do
          mocked = mock_result(command: %w[git version], stdout: "git version 2.39.1\n", stderr: "valid utf-8\n")
          allow(ProcessExecuter).to receive(:run_with_capture).and_return(mocked)
        end

        it 'preserves stdout unchanged' do
          expect(result.stdout).to eq("git version 2.39.1\n")
        end

        it 'preserves stderr unchanged' do
          expect(result.stderr).to eq("valid utf-8\n")
        end
      end

      context 'when output is valid but non-UTF-8 encoded' do
        before do
          # A Latin-1 (ISO-8859-1) string with a valid encoding but not UTF-8.
          # normalize_encoding re-encodes it to UTF-8.
          latin1_out = "caf\xE9 output\n".dup.force_encoding(Encoding::ISO_8859_1)
          latin1_err = "caf\xE9 warning\n".dup.force_encoding(Encoding::ISO_8859_1)
          mocked = mock_result(command: %w[git version], stdout: latin1_out, stderr: latin1_err)
          allow(ProcessExecuter).to receive(:run_with_capture).and_return(mocked)
        end

        it 'converts stdout to valid UTF-8' do
          expect(result.stdout).to eq("caf\u00E9 output\n")
          expect(result.stdout.encoding).to eq(Encoding::UTF_8)
        end

        it 'converts stderr to valid UTF-8' do
          expect(result.stderr).to eq("caf\u00E9 warning\n")
          expect(result.stderr.encoding).to eq(Encoding::UTF_8)
        end
      end
    end

    context 'with merge: true' do
      let(:run_opts) { { merge: true } }

      it 'forwards merge_output: true to ProcessExecuter' do
        expect(ProcessExecuter).to receive(:run_with_capture) do |*_env_and_cmd, **opts|
          expect(opts[:merge_output]).to be true
          mock_result(command: %w[git version])
        end
        result
      end
    end
    ...
    context 'with err: option' do
      ...
      it 'forwards err: to ProcessExecuter' do
        expect(ProcessExecuter).to receive(:run_with_capture) do |*_env_and_cmd, **opts|
          expect(opts[:err]).to be(err_io)
          mock_result(command: %w[git version])
        end
        result
      end
    ...
    context 'with env: option' do
      ...
      it 'merges the per-call env into the instance env and passes it as the first argument' do
        expect(ProcessExecuter).to receive(:run_with_capture) do |env_arg, *_cmd, **_opts|
      ...
      it 'attaches the result to the error' do
        expect { described_instance.run('status') }
          .to raise_error(Git::FailedError, /git.*status/) do |error|
            ...
            expect(error.result.stdout).to eq("Changes to be committed:\n  modified: foo.rb\n")
    ...
    context 'when command times out' do
      before do
        mocked = mock_result(command: %w[git status], success?: false, timed_out?: true)
        allow(ProcessExecuter).to receive(:run_with_capture).and_return(mocked)
      end
    ...
    context 'when command is signaled' do
      before do
        mocked = mock_result(command: %w[git status], success?: false, signaled?: true)
        allow(ProcessExecuter).to receive(:run_with_capture).and_return(mocked)
      end
    ...
    context 'when ProcessExecuter raises ArgumentError' do
      ...
      before do
        allow(ProcessExecuter).to receive(:run_with_capture).and_raise(pe_error)
      end
