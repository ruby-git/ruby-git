  ...
  describe '#call' do
    ...
    context 'with stash reference' do
      it 'pops specific stash by name' do
        expect_command_capturing('stash', 'pop', 'stash@{0}').and_return(command_result(''))
        command.call('stash@{0}')
      end

      it 'pops specific stash by index' do
        expect_command_capturing('stash', 'pop', 'stash@{2}').and_return(command_result(''))
        command.call('stash@{2}')
      end

      it 'pops stash using short form' do
        expect_command_capturing('stash', 'pop', '1').and_return(command_result(''))
        command.call('1')
      end
    end
    ...
    context 'with :index option' do
      it 'adds --index flag to restore index state' do
        expect_command_capturing('stash', 'pop', '--index').and_return(command_result(''))
