  ...
  describe '#call' do
    ...
    context 'with :message option' do
      it 'adds -m flag with message' do
        expect_command_capturing('stash', 'push', '--message', 'WIP changes').and_return(command_result(''))
    ...
    context 'with :staged option' do
      it 'adds -S flag to stash only staged changes' do
        expect_command_capturing('stash', 'push', '--staged').and_return(command_result(''))
    ...
    context 'with :pathspec_file_nul option' do
      it 'adds --pathspec-file-nul flag' do
        expect_command_capturing('stash', 'push', '--pathspec-from-file=paths.txt', '--pathspec-file-nul')
          .and_return(command_result(''))
    ...
    context 'with paths (pathspecs)' do
      it 'adds paths after -- separator' do
        expect_command_capturing('stash', 'push', '--', 'file.txt').and_return(command_result(''))
        command.call('file.txt')
      end

      it 'accepts multiple paths' do
        expect_command_capturing('stash', 'push', '--', 'file1.txt', 'file2.txt').and_return(command_result(''))
        command.call('file1.txt', 'file2.txt')
      end

      it 'combines paths with options' do
        expect_command_capturing('stash', 'push', '--message', 'Partial stash', '--', 'src/')
          .and_return(command_result(''))
    ...
    context 'with multiple options combined' do
      it 'combines keep_index with message' do
        expect_command_capturing('stash', 'push', '--keep-index', '--message', 'Testing')
          .and_return(command_result(''))
        command.call(keep_index: true, message: 'Testing')
      end

      it 'combines staged with paths' do
        expect_command_capturing('stash', 'push', '--staged', '--', 'src/', 'lib/')
          .and_return(command_result(''))
