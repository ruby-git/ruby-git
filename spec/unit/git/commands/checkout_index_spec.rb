  ...
  describe '#call' do
    context 'with no arguments' do
      it 'runs git checkout-index without any flags' do
        ...
        expect_command_capturing('checkout-index').and_return(expected_result)
    ...
    context 'with the :index option' do
      it 'includes --index when true' do
        expect_command_capturing('checkout-index', '--index').and_return(command_result)
    ...
    context 'with the :no_create option' do
      it 'includes --no-create when true' do
        expect_command_capturing('checkout-index', '--no-create').and_return(command_result)
    ...
    context 'with the :prefix option' do
      it 'includes --prefix=<value>' do
        expect_command_capturing('checkout-index', '--prefix=output/').and_return(command_result)
    ...
    context 'with the :temp option' do
      it 'includes --temp when true' do
        expect_command_capturing('checkout-index', '--temp').and_return(command_result)
    ...
    context 'with the :ignore_skip_worktree_bits option' do
      it 'includes --ignore-skip-worktree-bits when true' do
        expect_command_capturing('checkout-index', '--ignore-skip-worktree-bits').and_return(command_result)
    ...
    context 'with path_limiter positional arguments' do
      it 'appends -- and a single path' do
        expect_command_capturing('checkout-index', '--', 'file.txt').and_return(command_result)

        command.call('file.txt')
      end

      it 'appends -- and multiple paths when given as separate arguments' do
        expect_command_capturing('checkout-index', '--', 'file1.txt', 'file2.txt').and_return(command_result)
    ...
    context 'with multiple options combined' do
      it 'includes all specified flags' do
        expect_command_capturing('checkout-index', '--all', '--force', '--prefix=output/').and_return(command_result)
    ...
    context 'with :force and file operand combined' do
      it 'includes --force and appends -- path' do
        expect_command_capturing('checkout-index', '--force', '--', 'file.txt').and_return(command_result)
