  ...
  describe '#call' do
    context 'with nil tree_ish (restore from index)' do
      it 'omits tree_ish from command when nil' do
        ...
        expect_command_capturing('checkout', '--', 'file.txt').and_return(expected_result)
      ...
      it 'restores multiple files from index' do
        expect_command_capturing('checkout', '--', 'file1.txt', 'file2.txt').and_return(command_result)
        command.call(pathspec: ['file1.txt', 'file2.txt'])
      end

      it 'works with options' do
        expect_command_capturing('checkout', '--force', '--', 'file.txt').and_return(command_result)
    ...
    context 'with tree_ish and pathspec' do
      it 'places tree_ish before -- separator' do
        expect_command_capturing('checkout', 'HEAD~1', '--', 'file.txt').and_return(command_result)
        command.call('HEAD~1', pathspec: ['file.txt'])
      end

      it 'accepts branch name as tree_ish' do
        expect_command_capturing('checkout', 'main', '--', 'file.txt').and_return(command_result)
        command.call('main', pathspec: ['file.txt'])
      end

      it 'accepts commit SHA as tree_ish' do
        expect_command_capturing('checkout', 'abc123', '--', 'file.txt', 'other.txt').and_return(command_result)
        command.call('abc123', pathspec: %w[file.txt other.txt])
      end

      it 'accepts tag as tree_ish' do
        expect_command_capturing('checkout', 'v1.0.0', '--', 'config.yml').and_return(command_result)
        command.call('v1.0.0', pathspec: ['config.yml'])
      end

      it 'accepts glob patterns' do
        expect_command_capturing('checkout', 'HEAD', '--', '*.rb').and_return(command_result)
        command.call('HEAD', pathspec: ['*.rb'])
      end

      it 'accepts directory paths' do
        expect_command_capturing('checkout', 'HEAD', '--', 'src/').and_return(command_result)
    ...
    context 'with :pathspec_from_file option' do
      it 'adds --pathspec-from-file flag' do
        expect_command_capturing('checkout', '--pathspec-from-file=paths.txt', 'main').and_return(command_result)
        command.call('main', pathspec_from_file: 'paths.txt')
      end

      it 'accepts stdin with -' do
        expect_command_capturing('checkout', '--pathspec-from-file=-', 'HEAD').and_return(command_result)
    ...
    context 'with :pathspec_file_nul option' do
      it 'adds --pathspec-file-nul flag with pathspec_from_file' do
        expect_command_capturing('checkout', '--pathspec-from-file=paths.txt', '--pathspec-file-nul',
