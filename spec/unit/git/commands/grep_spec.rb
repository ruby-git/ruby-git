  ...
  describe '#call' do
    ...
    context 'with :exclude_standard option' do
      it 'includes --exclude-standard flag' do
        expect_command_capturing('grep', '--no-color', '--exclude-standard', '-e', 'search', 'HEAD')
          .and_return(command_result(''))

        command.call('HEAD', pattern: 'search', exclude_standard: true)
      end
    end

    context 'with :exclude_standard false' do
      it 'includes --no-exclude-standard flag' do
        expect_command_capturing('grep', '--no-color', '--no-exclude-standard', '-e', 'search', 'HEAD')
          .and_return(command_result(''))

        command.call('HEAD', pattern: 'search', exclude_standard: false)
      end
    end

    context 'with :cached option' do
      it 'includes --cached flag' do
        expect_command_capturing('grep', '--no-color', '--cached', '-e', 'search', 'HEAD')
          .and_return(command_result(''))

        command.call('HEAD', pattern: 'search', cached: true)
      end
    end

    context 'with :untracked option' do
      it 'includes --untracked flag' do
        expect_command_capturing('grep', '--no-color', '--untracked', '-e', 'search', 'HEAD')
          .and_return(command_result(''))

        command.call('HEAD', pattern: 'search', untracked: true)
      end
    end

    context 'with :no_index option' do
      it 'includes --no-index flag' do
        expect_command_capturing('grep', '--no-color', '--no-index', '-e', 'search', 'HEAD')
          .and_return(command_result(''))

        command.call('HEAD', pattern: 'search', no_index: true)
      end
    end
    ...
    context 'with :path_limiter option' do
      it 'appends -- and the path limiter after the tree operand' do
        expect_command_capturing('grep', '--no-color', '-e', 'search', 'HEAD', '--', 'lib/**')
          .and_return(command_result('HEAD:lib/file.rb:3:search'))

        command.call('HEAD', pattern: 'search', path_limiter: 'lib/**')
      end

      it 'appends -- and multiple path limiters when given an array' do
        expect_command_capturing('grep', '--no-color', '-e', 'search', 'HEAD', '--', 'lib/**',
                                 'spec/**')
          .and_return(command_result('HEAD:lib/file.rb:3:search'))

        command.call('HEAD', pattern: 'search', path_limiter: ['lib/**', 'spec/**'])
      end

      it 'omits -- separator when path_limiter is nil' do
        expect_command_capturing('grep', '--no-color', '-e', 'search', 'HEAD')
          .and_return(command_result(''))

        command.call('HEAD', pattern: 'search', path_limiter: nil)
      end
    end
