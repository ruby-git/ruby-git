...
RSpec.describe Git::Repository::Stashing do
  ...
  describe '#stash_store' do
    ...
    before do
      ...
      allow(rev_parse_command).to receive(:call).with(revision, verify: true).and_return(command_result("#{oid}\n"))
    ...
    it 'peels the commit to a commit id, stores the id, then lists the stash entries' do
      expect(rev_parse_command).to receive(:call).with(revision, verify: true)
    ...
    context 'when the commit is a :/<text> search' do
      let(:commit) { ':/wip' }
      let(:revision) { commit }

      it 'passes the search to rev-parse without the ^{commit} suffix' do
    ...
    context 'when the commit is not a String' do
      let(:commit) { build_stash_info(index: 1, oid: 'e' * 40) }
      let(:revision) { 'stash@{1}^{commit}' }

      it 'resolves its String form' do
        expect(store_command).to receive(:call).with(oid).and_return(store_result)
        result
      end
    end

    context 'when the commit does not resolve to a commit' do
      let(:commit) { 'refs/nope' }

      before do
        allow(rev_parse_command).to receive(:call).with(revision, verify: true).and_raise(
          Git::FailedError.new(command_result('', stderr: 'fatal: Needed a single revision', exitstatus: 128))
        )
      end

      it 'raises Git::FailedError without running the store command' do
        expect(store_command).not_to receive(:call)
        expect { result }.to raise_error(Git::FailedError, /Needed a single revision/)
      end
    end
    ...
    context 'when the listing has no entry for the commit' do
      ...
      it 'raises Git::UnexpectedResultError naming the commit id' do
        expect { result }.to raise_error(Git::UnexpectedResultError, /#{oid}/)
      end
    end
    ...
    context 'option whitelisting' do
      ...
      it 'reports the unknown option before validating the commit' do
        expect do
          described_instance.stash_store(nil, **options)
        end.to raise_error(ArgumentError, /Unknown options: bogus/)
      end
    ...
    context 'input validation' do
      shared_examples 'a commit rejected before git runs' do |message|
        it 'raises ArgumentError without running git' do
          expect(rev_parse_command).not_to receive(:call)
          expect(store_command).not_to receive(:call)
          expect { result }.to raise_error(ArgumentError, message)
        end
      end
      ...
      context 'when the commit is nil' do
        ...
        it_behaves_like 'a commit rejected before git runs', 'commit is required'
      end

      context 'when the commit starts with a hyphen' do
        let(:commit) { '-x' }

        it_behaves_like 'a commit rejected before git runs', "Invalid commit: '-x'"
      end

      context 'when the commit is a Symbol that starts with a hyphen' do
        let(:commit) { :'-x' }

        it_behaves_like 'a commit rejected before git runs', "Invalid commit: '-x'"
      end

      context 'when the commit is a negated revision' do
        let(:commit) { '^HEAD' }

        it_behaves_like 'a commit rejected before git runs', "Invalid commit: '^HEAD'"
      end
    end
  end
