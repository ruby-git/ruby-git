...
`Git::Error` if your code needs to handle them. Any method that captures git
output can also raise `Encoding::ConverterNotFoundError` when the output contains
a line whose detected encoding, such as UTF-7, has no Ruby converter.

Starting with v6.0.0, the filesystem and gzip errors are raised as `Git::Error`
with the original error available through `cause`, and lines with no Ruby
converter are scrubbed instead of raising.
