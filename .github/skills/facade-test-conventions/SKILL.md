...
`repo_dir` helpers. Facade integration specs must set `execution_context` to a
`Git::ExecutionContext::Repository`. Stage any required repository state in a
`before` block inside the spec itself.
