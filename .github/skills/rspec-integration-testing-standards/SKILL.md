...
- [Rules shared with unit specs](#rules-shared-with-unit-specs)
  ...
  - [Rule 4 (MUST): `require 'spec_helper'` and only the file(s) under test](#rule-4-must-require-spec_helper-and-only-the-files-under-test)
...
### Rule 4 (MUST): `require 'spec_helper'` and only the file(s) under test

Every spec starts with `require 'spec_helper'`, then requires only the Ruby file(s)
it directly tests. Do not require unrelated libraries or classes.
...
- **Every `Git.init` passes `initial_branch:`.** Without it, `HEAD` points at
  ...
  repositories created inside a `before` block.
- **Every repository the spec creates goes through `init_test_repo` or
  `pin_test_repo_config`.** These set `user.email`, `user.name`,
  `commit.gpgsign`, `core.editor`, and `core.autocrlf` so a commit succeeds without
  a global identity, never opens an editor, never signs, and never rewrites line
  endings on Windows. The `'in an empty repository'` shared context pins `repo`;
  any additional repository (a bare remote, a clone target, a submodule source)
  needs the same:

  ```ruby
  # Good
  let(:remote) { init_test_repo(remote_dir) }
  let(:bare)   { pin_test_repo_config(Git.init(bare_dir, bare: true, initial_branch: 'main')) }
...
- **No dependence on the machine's global git config.** A spec that needs a config
  value sets it on the test repository.
...
public facade on `repo` or through `execution_context.command_capturing`. It never
goes through a sibling command class, `system`, backticks, or `%x`:
...
(`write_file`, `create_directory`) or Ruby's standard library.
...
examples. Scope a change to a block (`Dir.chdir(dir) { ... }`) or save it in a
`let` and restore it in `after`. An example never leaves the process in a
...
The entry points in `lib/git.rb` map to `git_spec.rb` in the same directory. These
specs mirror a source file, so they need no header comment.
...
- **Parser specs** (`spec/integration/git/parsers/`) and the cross-cutting specs
  have no further conventions beyond Rules 1–21.
...
4. **Grep the file for the recurring deviations:** `raise_error(Git::FailedError)`
   ...
   (Rule 18, unless commented as a compare-against-git exception), `'/tmp'` /
   `'/dev/null'` (Rule 19), `sleep` (Rule 14).
