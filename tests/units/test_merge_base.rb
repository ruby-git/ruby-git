...
class TestMergeBase < Test::Unit::TestCase
  def test_branch_and_master_merge_base
    in_bare_repo_clone do |repo|
      ...
      assert_equal(ancestors.first, true_ancestor_sha) # proper common ancestor
    end
  end
  ...
  def test_branch_and_master_independent_merge_base
    in_bare_repo_clone do |repo|
      ...
      assert_equal(independent_commits.sort, true_independent_commits_shas.sort)
    end
  end
  ...
  def test_branch_and_master_all_merge_base
    in_bare_repo_clone do |repo|
      ...
      assert(true_ancestors_shas.include?(ancestors.first))
      ...
      assert_equal(all_ancestors.sort, true_ancestors_shas.sort)
    end
  end
  ...
  def test_branches_and_master_merge_base
    in_bare_repo_clone do |repo|
      ...
      assert_equal(ancestors.size, 1) # there is only one true ancestor
      assert_equal(ancestors.first, non_octopus_ancestor_sha) # proper common ancestor
    end
  end
